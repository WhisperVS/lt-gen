...
const noDupesEl = document.getElementById("noDupes");
const sortMainEl = document.getElementById("sortMain");
...
// Quick Pick preset buttons (1/5/10/20)
...
document.getElementById("minus").addEventListener("click", () => stepCount(-1));
document.getElementById("plus").addEventListener("click", () => stepCount(1));
...
generateBtn.addEventListener("click", () => {
  ...
  // Prevent duplicates across the batch
  // We'll generate tickets until we have n unique ones (with a retry cap).
  const uniqueSet = new Set();
  const tickets = [];
  const maxAttempts = Math.max(2000, n * 200); // safety cap
  ...
  while (tickets.length < n && attempts < maxAttempts) {
    ...
    // Key that defines a "ticket" for uniqueness
    // If sortMain is off and noDupes is off, order matters; but for lottery picks,
    // order doesn't matter. We'll treat order as irrelevant by sorting a copy for the key.
  ...
  if (tickets.length < n) {
    // This is extremely unlikely at these ranges, but we handle it cleanly.
    metaEl.textContent = `${rules.name} • generated ${tickets.length}/${n} unique ticket(s) (retry cap hit)`;
  }
...
function renderMeta(hasNew = false) {
  const rules = RULES[currentLottery];
  if (currentTickets.length === 0) {
    metaEl.textContent = `Selected: ${rules.name}. No tickets yet.`;
    return;
  }
  metaEl.textContent = `${rules.name} • ${currentTickets.length} ticket(s) • ${hasNew ? "Generated just now" : "Loaded"}`;
}
  ...
  for (const t of currentTickets) {
    ...
    rightBtn.addEventListener("click", async () => {
      ...
      try {
        ...
        window.prompt("Copy (Ctrl+C, Enter):", single);
      }
    });
...
function formatForCopy(tickets) {
  ...
  return tickets
    .map(t => `${rules.name} #${t.index}: ${t.main.join("-")} | ${rules.bonusName}: ${t.bonus}`)
    .join("\n");
}
